    ...
    return (<>
        <div className={`${index > 0 ? 'block' : 'hidden'} fixed top-0 left-0 right-0 bottom-0 bg-red-500 z-[999] bg-opacity-40`}>
            <div className={`flex w-full absolute top-0 left-0 right-0 bottom-0 z-[999] bg-slate-700 bg-opacity-40 justify-center items-center`}>
                <IoMdClose onClick={() => setIndex(0)} size={50} cursor={'pointer'} className={`absolute top-5 right-5 hover:scale-110 hover:rotate-90 duration-300`} />
                <form action={action} className=" w-1/2 h-1/2">
                    ...
                    <button value={index} name="index" disabled={isPending} type='submit' className='bg-emerald-500 rounded-lg px-10 w-full py-2 text-lg mt-5 hover:scale-110 hover:tracking-widest duration-300'>Edit Blog</button>
                </form>
            </div>
        </div>
    </>)
